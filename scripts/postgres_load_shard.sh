...
# master_node=$(getent hosts tasks.master | awk '{print $1}'| head -n 1)
# shard_nodes=(`getent hosts tasks.shard | awk '{print $1}'`)
# port="5432"

# db_url="jdbc:postgresql://$master_node:$port/ycsb?reWriteBatchedInserts=true;"
# shards_count=${#shard_nodes[@]}

# for (( i=0; i<$shards_count-1; i++ ))
# do
#     db_url+="jdbc:postgresql://${shard_nodes[$i]}:$port/ycsb?reWriteBatchedInserts=true;"
# done
# db_url+="jdbc:postgresql://${shard_nodes[$i]}:$port/ycsb?reWriteBatchedInserts=true"
...
$YCSB_HOME/bin/ycsb load jdbc -s -P $YCSB_HOME/workloads/workload$workload -P $JDBC_HOME/conf/db.properties -cp $CLASSPATH -p recordcount=$recordcount -p operationcount=10000 > /ycsb_outputs/postgres-out-load-$replicas-$recordcount-$workload-bs$b_size-fs$f_size.txt 2>&1

chmod 777 /ycsb_outputs/postgres-out-load-$replicas-$recordcount-$workload-bs$b_size-fs$f_size.txt
